...
(function () {
  ...
  const INDEX_SNAPSHOT_SCHEMA = 'chatharbor-index-v1';
  ...
  const readIndexSnapshot = (storage, { platform, scopeKey }) => { try { const snapshot = storage?.read?.(); if (!snapshot || snapshot.schemaVersion !== INDEX_SNAPSHOT_SCHEMA || snapshot.platform !== platform || snapshot.scopeKey !== scopeKey || !Array.isArray(snapshot.conversations) || !Array.isArray(snapshot.probe) || snapshot.conversations.some(item => !item?.conversationId || item.platform !== platform)) return null; return snapshot; } catch { return null; } };
  const loadIndexedConversations = async ({ adapter, storage, scopeKey = 'default', pageSize = 20, maxPages = 1000, force = false, now = () => new Date().toISOString(), onProgress = () => {} }) => { const cached = force ? null : readIndexSnapshot(storage, { platform: adapter.platform, scopeKey }); onProgress({ status: 'probing' }); const first = await adapter.listPage({ offset: 0, limit: pageSize }); const probe = firstPageProbe(first.items); if (cached && sameProbe(probe, cached.probe)) return { conversations: cached.conversations, source: 'validated-cache', probe }; const deduped = new Map(); let page = first; let offset = 0; for (let pageNumber = 0; pageNumber < maxPages; pageNumber++) { const items = page.items || []; for (const item of items) deduped.set(identity(item), item); onProgress({ status: 'syncing', page: pageNumber + 1, count: deduped.size }); offset = Number.isInteger(page.offset) ? page.offset + items.length : offset + items.length; const total = Number.isInteger(page.total) ? page.total : null; if (!items.length || (total !== null && offset >= total) || (total === null && items.length < pageSize)) break; if (pageNumber + 1 >= maxPages) throw new Error('Conversation index max-page guard reached'); page = await adapter.listPage({ offset, limit: pageSize }); } const conversations = [...deduped.values()]; const snapshot = { schemaVersion: INDEX_SNAPSHOT_SCHEMA, platform: adapter.platform, scopeKey, verifiedAt: now(), conversations, probe }; storage?.write?.(snapshot); return { conversations, source: force ? 'full-refresh' : cached ? 'refreshed-cache' : 'cache-miss', probe }; };
  ...
  const adapter = {
    ...
    async listPage({ offset = 0, limit = 20 } = {}) {
      const h = await headers();
      const r = await fetch(`/backend-api/conversations?offset=${offset}&limit=${limit}&order=updated`, { headers: h });
      if (!r.ok) { const error = new Error(`Conversation list failed: ${r.status}`); error.status = r.status; throw error; }
      const j = await r.json();
      return { items: (j.items || []).map(x => normalize(x, { platform: 'chatgpt', conversationId: x.id, title: x.title, createdAt: x.create_time, updatedAt: x.update_time })), offset, total: Number.isInteger(j.total) ? j.total : null };
    },
  ...
  const run = async () => {
    ...
    panel.innerHTML = `<header style="grid-column:1/-1;display:flex;justify-content:space-between;align-items:center"><div><strong style="font-size:20px">ChatHarbor Pilot Workspace</strong><div data-role="status" style="color:#666;font-size:12px">ChatGPT · ${indexSource === 'validated-cache' ? '已验证缓存' : `已同步 ${list.length} 条`}</div></div><button data-role="close">关闭</button></header><main data-role="main" style="display:flex;flex-direction:column;gap:10px"><input data-role="search" placeholder="搜索标题或 Conversation ID"><div data-role="list" style="border:1px solid #ddd;border-radius:8px"></div></main><aside data-role="rail" style="border:1px solid #ddd;border-radius:8px;padding:12px;gap:12px"><div data-role="summary"></div><div><strong>导出策略</strong><div data-role="strategy">当前 Pilot · 单批</div></div><button data-role="refresh">完整刷新</button><div data-role="progress" hidden></div><button data-role="cancel" hidden>取消</button><button data-role="retry" hidden>重试失败项</button><button data-role="export" disabled style="padding:10px;border:0;border-radius:8px;background:#10a37f;color:#fff;font-weight:700">导出选中 0 条</button></aside>`;
