...
(function () {
  ...
  const retryOperation = async (operation, { maxAttempts = 3, sleep = ms => new Promise(resolve => setTimeout(resolve, ms)), random = Math.random, isCancelRequested = () => false, refreshAuth = null, onRetry = () => {} } = {}) => { let refreshedAuth = false; let lastError; for (let attempt = 1; attempt <= maxAttempts; attempt++) { if (isCancelRequested()) throw new Error('Retry cancelled'); try { return await operation(); } catch (error) { lastError = error; const status = Number.isInteger(error?.status) ? error.status : null; if ((status === 401 || status === 403) && attempt < maxAttempts && !refreshedAuth && typeof refreshAuth === 'function') { refreshedAuth = true; if (await refreshAuth()) { if (isCancelRequested()) throw new Error('Retry cancelled'); continue; } throw error; } if (!(status === null || status >= 500) || attempt === maxAttempts) throw error; const delay = (2 ** attempt) * 1000 + Math.floor(random() * 501); onRetry({ attempt, delay, error }); await sleep(delay); if (isCancelRequested()) throw new Error('Retry cancelled'); } } throw lastError; };
  ...
  const run = async () => {
    ...
    const executeTargets = async targets => { if (executionActive || !targets.length) return; executionActive = true; executionStatus = 'running'; lastTargets = [...targets]; render(); controller = createExecutionController(targets, async (conversation, context) => { const c = await retryOperation(() => adapter.fetchConversation(conversation.conversationId), { isCancelRequested: context.isCancelRequested, refreshAuth: async () => { accessToken = null; try { await token(true); return true; } catch { return false; } } }); const out = exportPair(c); download(`chatharbor-${c.conversationId}.json`, out.json, 'application/json'); download(`chatharbor-${c.conversationId}.md`, out.md, 'text/markdown'); console.info('[ChatHarbor Pilot] selected export', c, out.manifest); }, showProgress); try { const result = await controller.run(); lastFailures = result.failures; } finally { executionActive = false; render(); } };
    retryBtn.onclick = () => { const failed = new Set(lastFailures.map(failure => failure.identity)); const targets = lastTargets.filter(target => failed.has(target.identity)); if (targets.length !== failed.size) throw new Error('Failed conversation identity could not be resolved'); executeTargets(targets); };
