...
(function () {
  ...
  const run = async () => {
    ...
    panel.innerHTML = `<header style="grid-column:1/-1;display:flex;justify-content:space-between;align-items:center"><div><strong style="font-size:20px">ChatHarbor Pilot Workspace</strong><div data-role="status" style="color:#666;font-size:12px">ChatGPT · Test/Pilot</div></div><button data-role="close">关闭</button></header><main data-role="main" style="display:flex;flex-direction:column;gap:10px"><input data-role="search" placeholder="搜索标题或 Conversation ID"><div data-role="list" style="border:1px solid #ddd;border-radius:8px"></div></main><aside data-role="rail" style="border:1px solid #ddd;border-radius:8px;padding:12px;gap:12px"><div data-role="summary"></div><div><strong>导出策略</strong><div data-role="strategy">当前 Pilot · 单批</div></div><button data-role="export" disabled style="padding:10px;border:0;border-radius:8px;background:#10a37f;color:#fff;font-weight:700">导出选中 0 条</button></aside>`;
    ...
    const render = () => { const q = search.value.trim().toLowerCase(); filtered = list.filter(c => !q || `${c.title} ${c.identity}`.toLowerCase().includes(q)); listEl.innerHTML = ''; filtered.forEach(c => { const row = document.createElement('label'); Object.assign(row.style, { display: 'flex', gap: '8px', padding: '10px', borderBottom: '1px solid #eee', cursor: 'pointer' }, layout.row); row.innerHTML = `<input type="checkbox" ${selected.has(c.identity) ? 'checked' : ''}><span>${c.title || '(未命名)'} <small style="color:#777">${c.conversationId}</small></span>`; Object.assign(row.querySelector('span').style, layout.title); row.querySelector('input').onchange = e => { e.target.checked ? selected.add(c.identity) : selected.delete(c.identity); render(); }; listEl.appendChild(row); }); summary.textContent = `已选 ${selected.size} 条 · 匹配 ${filtered.length} / 总计 ${list.length}`; exportBtn.textContent = `导出选中 ${selected.size} 条`; exportBtn.disabled = selected.size === 0; };
