...
## TASK-006C Archive Coverage Remediation — Independent Review PASS

Independent Review passed personal/root active-plus-archived serial enumeration, Adapter-owned archive endpoint context, stable identity dedupe, cache schema v2, compound active/archive probe semantics, and the default unarchived archive filter. `updatedAt` and available totals remain index-refresh hints only. Project-space enumeration and ownership remain TASK-003 Partial/Unknown. Archive-focused live re-verification is deferred and will be combined with the accepted workspace-layout integrated milestone.

## TASK-006D Accepted Workspace Layout Alignment — implementation and automated verification

The original TASK-006A Pilot was a minimum functional workspace slice and its Human Browser PASS remains valid for that slice. Human Owner feedback identified that the temporary shell was structurally partial against the already accepted DEC-0001/SPEC information architecture. No new product decision was required: DEC-0001 and the CHG-0002 Specification remain authoritative.

`ChatHarbor/ui/workspace.js` now defines a machine-checkable workspace surface: toolbar order is Search, Scope, Archive, Export Status, Time, Refresh; rail order is Selection Summary, Export Strategy, Record Management, Progress, Retry, Export Actions. The Pilot exposes the same descriptor through its inert VM hook. Scope stays visibly unavailable because ChatGPT scope capability is not established; Archive is active; export status, time, record management, current-filter export, and current-scope export are unavailable placeholders rather than false runtime claims. Full Refresh moved from the rail to the toolbar. The active selected-only export, Cancel danger styling, progress, retry, archive/search filtering, selection identity, and containment contracts are retained.

Local tests directly validate toolbar/rail ordering, capability and availability states, source/dist surface parity, selected-export zero-selection protection, and existing workspace/index/execution regressions. Human Browser Verification is not performed or claimed for TASK-006D; Independent Review is pending. No GitHub CI claim is made.
