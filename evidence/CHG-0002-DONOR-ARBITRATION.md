...
|---|---|---|---|---|---|---|---|---|
| List loading / pagination | Source-mature ChatGPT root/project/archive paging | Source-mature multi-platform adapter paging | Indexed serial pagination implemented; Independent Review pending | MERGE | Adapter / Index | Preserve generic adapter seam; adapt v0.4 ChatGPT pagination inside ChatGPT adapter | Page continuation, dedupe, bare ID mapping | Yes, integrated multi-page ChatGPT milestone |
| Cache | Source-mature local list snapshot | Settings/count caches only | Validated metadata snapshot implemented; Independent Review pending | ADAPT | Core Index/Cache | Extract v0.4 snapshot structure without UI; preserve scope keys | Cache read/write, stale fallback | Yes, cache/refresh milestone |
| Incremental detection | First-page `(id, update_time)` comparison | No comparable mature path | First-page probe hint implemented; Independent Review pending | ADAPT | Adapter / Index | Use only as refresh hint; never feed version state | Equal/different page hints; no version-state mutation | Yes, refresh milestone |
...
| Archive | Source-mature root active/archived list routes | Adapter-dependent metadata | Personal/root archive enumeration implemented; Project ownership remains TASK-003 Partial/Unknown; Independent Review pending | ADAPT | ChatGPT adapter / Index | Adapt v0.4 root partition enumeration only; do not infer Project ownership | Active/archived/unknown mapping, compound cache probe | Yes, focused personal/root archive milestone |
...
This is a planning control only. TASK-006B1 Automated Review PASS and Human Browser Verification PASS are recorded; no repeat B1 browser verification is required. Capability Donor Arbitration Independent Review = PASS. TASK-006B2 implementation, automated verification, and Independent Review PASS are recorded; live failure-path browser evidence remains Deferred/Unknown. TASK-006C follows the approved pagination/cache/incremental donor decisions and awaits its own Independent Review; later capabilities remain prohibited.
