...
| Retry | ChatGPT detail retry + token refresh | Exponential backoff + callbacks | Implemented; Independent Review remediation pending | MERGE | Core retry + ChatGPT adapter | Generic backoff policy; v0.4 auth-refresh belongs in adapter | Retry limit/backoff, retryable network/5xx behavior, one auth-refresh attempt for 401/403, terminal auth failure after refresh fails or repeated auth failure, failure identity retention, cooperative cancellation during retry/backoff | Yes, retry workflow milestone |
