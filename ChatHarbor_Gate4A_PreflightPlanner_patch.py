...
if len(sys.argv) < 2:
    print("Usage: python ChatHarbor_Gate4A_PreflightPlanner_patch.py <Tampermonkey.js> [output.user.js]")
    raise SystemExit(2)

src = Path(sys.argv[1])
out = Path(sys.argv[2]) if len(sys.argv) > 2 else src.with_name("ChatHarbor-Gate4A.user.js")
raw = src.read_bytes()
...
text = text.replace(
    ...
    "// @author       huhu\n",
    "// @name         ChatHarbor Gate 4A (Clean Lineage)\n"
    "// @name:zh-CN   ChatHarbor Gate 4A（干净来源预检版）\n"
    "// @version      0.0.4.0\n"
    "// @description  Clean-lineage read-only archive scan and preflight planner based on huhusmang/ChatGPT-Exporter.\n"
    "// @description:zh-CN 基于 huhusmang/ChatGPT-Exporter 的干净来源本地档案扫描与只读预检规划版。\n"
...
directory_writer = r'''
    ...
    //
    // Gate 4A preserves the validated Gate 3 / 3.1 writer unchanged, then adds a
    ...
    // ======================== ChatHarbor Gate 4A: Read-only Archive Scan + Preflight Planner ========================
    ...
    async function chCollectPreflightRemoteUniverse(mode, workspaceId, currentList) {
        ...
        try {
            ...
            if (mode === 'personal') {
                ...
                return {
                    remoteList: base.concat(projectList),
                    complete: true,
            ...
            if (mode === 'project') {
                ...
                return {
                    remoteList: rootList.concat(base),
                    complete: true,
    ...
    async function chRunPreflightPlanner({ rootHandle, remoteList, selectedIds = null, remoteUniverseComplete = true, remoteUniverseNote = null }) {
        ...
        console.log('[ChatHarbor Gate 4A] Preflight plan (read-only):', { localScan, plan });
...
preflight_handler = r'''            preflightBtn.onclick = async () => {
                ...
                try {
                    ...
                    console.error('[ChatHarbor Gate 4A] Preflight failed:', err);
