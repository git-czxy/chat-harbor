...
if (-not $python) {
    ...
    & $python.Source -3 (Join-Path $Root 'tests\test_ui_010.py')
} else {
    & $python.Source (Join-Path $Root 'tests\test_ui_010.py')
}
...
Write-Host 'All ChatHarbor 0.0.10.0 automated regressions passed.' -ForegroundColor Green
