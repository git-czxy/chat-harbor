...
if (-not $python) {
    ...
    & $python.Source -3 (Join-Path $Root 'tests\test_ui_0130.py')
    if ($LASTEXITCODE -ne 0) { throw 'UI/static regression failed.' }
    & $python.Source -3 (Join-Path $Root 'tests\test_release_0130.py')
} else {
    & $python.Source (Join-Path $Root 'tests\test_ui_0130.py')
    if ($LASTEXITCODE -ne 0) { throw 'UI/static regression failed.' }
    & $python.Source (Join-Path $Root 'tests\test_release_0130.py')
}
if ($LASTEXITCODE -ne 0) { throw 'Release invariant regression failed.' }
& $python.Source (Join-Path $Root 'tests\test_scheduler_0130.py')
...
Write-Host 'All ChatHarbor 0.0.13.0 automated regressions passed.' -ForegroundColor Green
