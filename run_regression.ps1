...
if (-not $python) {
    ...
    & $python.Source -3 (Join-Path $Root 'tests\test_ui_0111.py')
} else {
    & $python.Source (Join-Path $Root 'tests\test_ui_0111.py')
}
...
Write-Host 'All ChatHarbor 0.0.11.1 automated regressions passed.' -ForegroundColor Green
