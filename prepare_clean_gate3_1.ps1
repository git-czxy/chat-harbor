...
$Output = Join-Path $PSScriptRoot "ChatHarbor-Gate3.1.user.js"
$Patcher = Join-Path $PSScriptRoot "ChatHarbor_Gate3_1_ManifestSafety_patch.py"

Write-Host "[1/3] Downloading fixed clean upstream..."
Invoke-WebRequest -Uri $Url -OutFile $Upstream

Write-Host "[2/3] Applying ChatHarbor Gate-3.1 patch..."
python $Patcher $Upstream $Output
if ($LASTEXITCODE -ne 0) { throw "Patcher failed." }

Write-Host "[3/3] Done."
Write-Host "Generated: $Output"
Write-Host "Install it in Tampermonkey and test against a copied/disposable archive directory."
