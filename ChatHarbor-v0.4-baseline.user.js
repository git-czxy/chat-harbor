// ==UserScript==
// @name         ChatGPT Universal Exporter (Markdown Support) - Conservative Edition
// @name:zh-CN   ChatGPT 对话导出器 - 超保守版
// @name:zh-TW   ChatGPT 对话匯出器 - 超保守版
// @namespace    https://greasyfork.org/users/1616937-owlct
// @version      1.4.1-conservative
// @description  Ultra-conservative ChatGPT exporter with speed levels, deduplication, auto-batch resume, max-per-batch limit and safety confirms.
// @description:zh-CN  超保守版：多档速度、本地去重、自动分批续传、单次上限、二次确认，最大限度降低风控风险。
// @description:zh-TW  超保守版：多檔速度、本地去重、自動分批續傳、單次上限、二次確認，最大限度降低風控風險。
// @author       OwlCt + Conservative Patch
// @license      MIT
...
// v1.4.1 更新（2026）
//  - 修复多选导出 bug：选中对话数超过单次上限（MAX_EXPORT_PER_BATCH）时，不再截断为单批，改为放行全量交给底层分批引擎分多批导出。
//  - 二次确认文案改为中文双语提示，明确「将按上限拆分 N 批全部导出」；覆盖 alert.batchLimit 文案。
